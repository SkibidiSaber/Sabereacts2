...
const MAX_WHITELISTED_USERS = 100;
...
function normalizeSetting(setting) {
  ...
  if (setting?.userId && setting?.emoji) {
    return {
      ...
      mode: "all_configured",
      whitelistUserIds: uniqueArray(Array.isArray(setting.whitelistUserIds) ? setting.whitelistUserIds : []).slice(0, MAX_WHITELISTED_USERS),
...
const commands = [
  new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((opt) =>
          opt
            .setName("emojis")
            .setDescription("Emojis to save: 👍, <:name:id>, or :serverEmojiName:. Optional for standard/server modes.")
            .setRequired(false)
            .setMaxLength(1500)
        )
        .addStringOption((opt) =>
    ...
    .addSubcommand((sub) =>
      sub
        .setName("save")
        .setDescription("Save emojis for all-configured or random-configured mode.")
        .addStringOption((opt) =>
          opt
            .setName("emojis")
            .setDescription("Emojis to save: 👍, <:name:id>, <a:name:id>, or :serverEmojiName:.")
            .setRequired(true)
            .setMaxLength(1500)
        )
    )
    ...
    .addSubcommand((sub) =>
      sub
        .setName("clear")
        .setDescription("Clear users, saved emojis, whitelist, or everything.")
        .addStringOption((opt) =>
          opt
            .setName("target")
    ...
    .addSubcommand((sub) =>
      sub.setName("off").setDescription("Disable reactions while keeping the access whitelist.")
    )
    .addSubcommand((sub) =>
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .setDescription("Allow listed members to use autoreact commands.")
        .addStringOption((opt) =>
          opt
            .setName("users")
            .setDescription("Members to whitelist. Mention users or paste IDs.")
            .setRequired(true)
            .setMaxLength(1500)
        )
    )
    .addSubcommand((sub) =>
      sub
        ...
        .setDescription("Remove listed members from the command whitelist.")
        .addStringOption((opt) =>
          opt
            .setName("users")
            .setDescription("Members to remove from the whitelist. Mention users or paste IDs.")
            .setRequired(true)
            .setMaxLength(1500)
        )
    )
    ...
    .addSubcommand((sub) =>
      sub.setName("whitelist-status").setDescription("Show who is whitelisted to use commands.")
    ),
].map((cmd) => cmd.toJSON());
...
function isWhitelisted(interaction) {
  const current = normalizeSetting(settings[interaction.guild.id]);
  return Boolean(current?.whitelistUserIds?.includes(interaction.user.id));
}

function canUseAutoreactCommands(interaction) {
  return isOwner(interaction) || isAdministrator(interaction) || isWhitelisted(interaction);
}

function canManageWhitelist(interaction) {
  return isOwner(interaction) || isAdministrator(interaction);
}

function denyCommandAccess(interaction) {
  interaction.reply({
    content: "You need to be the server owner, have Administrator permission, or be whitelisted to use this bot's commands.",
...
function denyWhitelistManagement(interaction) {
  interaction.reply({
    content: "Only the server owner or members with Administrator permission can manage the command whitelist.",
    flags: MessageFlags.Ephemeral,
  });
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  const whitelistManagementSubcommands = new Set([
    "whitelist-add",
    "whitelist-remove",
    "whitelist-clear",
    "whitelist-status",
  ...
  if (whitelistManagementSubcommands.has(subcommand)) {
    if (!canManageWhitelist(interaction)) return denyWhitelistManagement(interaction);
  } else if (!canUseAutoreactCommands(interaction)) {
    return denyCommandAccess(interaction);
  }
  ...
  if (subcommand === "save") {
    ...
    const savedCount = current.emojis.length - oldEmojiCount;
    if (parsedEmojis.truncated) warnings.push(`Only the first ${MAX_CONFIGURED_EMOJIS} emojis from this command were processed.`);
    if (current.mode === "all_configured" && current.emojis.length > MAX_REACTIONS_PER_MESSAGE) {
      warnings.push(`all-configured mode reacts with the first ${MAX_REACTIONS_PER_MESSAGE} saved emojis per message.`);
    }
    ...
    return interaction.reply({
      content: [
        `Saved ${savedCount} new emoji reaction(s). Existing saved emojis were kept.`,
        `Saved emojis now: ${current.emojis.length} (${summarizeList(current.emojis)}).`,
        `Current mode: ${formatMode(current.mode)}.`,
        warnings.length ? `Warnings: ${warnings.join(" ")}` : null,
      ]
        .filter(Boolean)
        .join("\n"),
  ...
  if (subcommand === "clear") {
    ...
    if ((target === "whitelist" || target === "all") && !canManageWhitelist(interaction)) {
      return denyWhitelistManagement(interaction);
    }
    ...
    if (target === "users") {
      ...
      saveSettings(settings);
      return interaction.reply({
        content: "Cleared users, saved emojis, mode, and the command whitelist for this server.",
        flags: MessageFlags.Ephemeral,
      });
    }
  ...
  if (subcommand === "off") {
    ...
    current.userIds = [];
    current.emojis = [];
    current.mode = "all_configured";
    current.version = SETTINGS_VERSION;
    current.updatedAt = new Date().toISOString();
    saveGuildSetting(guildId, current);

    return interaction.reply({
      content: "Disabled automatic reactions by clearing watched users and saved emojis. The command whitelist was kept.",
      flags: MessageFlags.Ephemeral,
    });
  }
  ...
  if (subcommand === "status") {
    ...
    if (!current) {
      return interaction.reply({
        content: "Automatic reactions are currently disabled for this server. Access: server owner and Administrators only until a whitelist is added.",
        flags: MessageFlags.Ephemeral,
      });
    }

    const serverEmojiCount = (await getGuildReactionEmojis(interaction.guild)).length;
    const hasReactionTargets = current.userIds.length > 0;
    const hasConfiguredReactionSource =
      current.mode === "random_standard" ||
      current.mode === "random_server" ||
      current.mode === "random_any" ||
      current.emojis.length > 0;

    return interaction.reply({
      content: [
        `Automatic reactions: ${hasReactionTargets && hasConfiguredReactionSource ? "enabled" : "disabled / incomplete"}.`,
        `Users watched: ${current.userIds.length} (${summarizeList(current.userIds, (id) => `<@${id}>`)}).`,
        `Saved emojis: ${current.emojis.length} (${summarizeList(current.emojis)}).`,
        `Mode: ${formatMode(current.mode)}.`,
        `Command whitelist: ${current.whitelistUserIds.length} (${summarizeList(current.whitelistUserIds, (id) => `<@${id}>`)}).`,
        `Administrators: allowed automatically.`,
  ...
  if (subcommand === "whitelist-add") {
    ...
    if (userIdsToAdd.length === 0) {
      return interaction.reply({
        content: "No valid users were found. Mention members or paste raw Discord user IDs.",
        flags: MessageFlags.Ephemeral,
      });
    }

    const current = getGuildSettingOrEmpty(guildId);
    const oldWhitelistCount = current.whitelistUserIds.length;

    for (const userId of userIdsToAdd) uniquePush(current.whitelistUserIds, userId);
    current.whitelistUserIds = current.whitelistUserIds.slice(0, MAX_WHITELISTED_USERS);
    current.version = SETTINGS_VERSION;
    current.updatedAt = new Date().toISOString();
    saveGuildSetting(guildId, current);

    const addedCount = current.whitelistUserIds.length - oldWhitelistCount;

    return interaction.reply({
      content: [
        `Whitelisted ${addedCount} new member(s).`,
        `Command whitelist now: ${current.whitelistUserIds.length} (${summarizeList(current.whitelistUserIds, (id) => `<@${id}>`)}).`,
        userIdsToAdd.length >= MAX_WHITELISTED_USERS ? `Only the first ${MAX_WHITELISTED_USERS} users from this command were processed.` : null,
      ]
        .filter(Boolean)
        .join("\n"),
      flags: MessageFlags.Ephemeral,
  ...
  if (subcommand === "whitelist-remove") {
    ...
    const current = getGuildSettingOrEmpty(guildId);
    const oldWhitelistCount = current.whitelistUserIds.length;
    current.whitelistUserIds = removeItems(current.whitelistUserIds, userIdsToRemove);
    current.version = SETTINGS_VERSION;
    current.updatedAt = new Date().toISOString();
    saveGuildSetting(guildId, current);

    const removedCount = oldWhitelistCount - current.whitelistUserIds.length;

    return interaction.reply({
      content: [
        `Removed ${removedCount} member(s) from the command whitelist.`,
        `Command whitelist now: ${current.whitelistUserIds.length} (${summarizeList(current.whitelistUserIds, (id) => `<@${id}>`)}).`,
      ].join("\n"),
      flags: MessageFlags.Ephemeral,
    });
  ...
  if (subcommand === "whitelist-clear") {
    ...
    return interaction.reply({
      content: "Cleared the command whitelist. The server owner and Administrators can still use commands.",
      flags: MessageFlags.Ephemeral,
    });
  }
  ...
  if (subcommand === "whitelist-status") {
    ...
    return interaction.reply({
      content: [
        `Command whitelist: ${current.whitelistUserIds.length} (${summarizeList(current.whitelistUserIds, (id) => `<@${id}>`)}).`,
        "Server owner and Administrators are always allowed.",
      ].join("\n"),
      flags: MessageFlags.Ephemeral,
    });
  }
